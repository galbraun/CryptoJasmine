...
void uart_print_no_eol(char* string)
{
	while (1)
	{
		uart_txbyte(*string);
...
void uart_scan(char* string, int size)
{ int i=0;
	while (i<size)
	{
		string[i]=uart_rxbyte();
...
#include <stdarg.h>
int string_to_data(char * msg,int size)
{
	 	int i=0; 
		char ch;
        uart_scan(&ch,1);
...
void uart_printf_no_eol(const char * msg, ...)
{	
 char out[512];
 size_t size = 512;
 int n = 0; 
